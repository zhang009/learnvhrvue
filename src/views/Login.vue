    ...
    <div>
        <el-form
            ...
            <el-form-item prop="password">
                <el-input size="normal" type="text" v-model="loginForm.password" auto-complete="off" placehoder="请输入密码：" @keydown.enter.native="submitLogin"></el-input>
            </el-form-item>
            ...
            submitLogin(){
                ...
                this.$refs.loginForm.validate((valid) => {//this.refs可以获取到当前页面所有的ref
                    if (valid) {
                        ...
                        postKeyValueRequest('/doLogin',this.loginForm).then(resp=>{//then里面的参数即是服务端返回的参数，（被处理过的）
                                if(resp){
                                    this.loading=false;
                                   // alert(JSON.stringify(resp));
                                    //这里用户的数据保存在js的sessionStorage里面，sessionStorage打开页面关掉就没了
                                    window.sessionStorage.setItem("user",JSON.stringify(resp.obj));//第二个参数只能保存字符串
